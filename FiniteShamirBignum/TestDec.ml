...
let test_mod_poly_elts () =
  let prime = (fromInt 1279) in
  let poly1 = [(fromInt 1456); (fromInt 1500); (fromInt 66)] in
  assert(mod_poly_elts prime poly1 =
      [(fromInt 177); (fromInt 221); (fromInt 66)]);
  let poly2 = [(fromInt (-1456)); (fromInt (-1500)); (fromInt (-66))] in
  assert(mod_poly_elts prime poly2 =
      [(fromInt 1102); (fromInt 1058); (fromInt 1213)]);      
;;

let test_mult_mod_inverse () =
  let a = (fromInt 1279) in
  let b = 6 in
  let inv = mult_mod_inverse a b in
  assert(snd(divmod (times inv (fromInt b)) a) = (fromInt 1)); 
;;
...
let test_decode_keys () =
  ...
  let key1 = (2, (fromInt 663)) in
  let key2 = (4, (fromInt 844)) in
  let key3 = (5, (fromInt 577)) in
  let keys = [key1; key2; key3] in
  assert(decode_keys prime keys = poly)
;;
...
let test_get_secret () =
  ...
  let key3 = (5, (fromInt 577)) in
  let keys = [key1; key2; key3] in
  assert(get_secret prime keys = secret)
;;
