...
include FiniteShamirInt_decode
...
let test_extended_euclidean () =
  let a = 1 in
  let b = 0 in
  assert(extended_euclidean a b = (1,1,0));
  let a = 9 in
  let b = 3 in
  assert(extended_euclidean a b = (3,0,1));
  let a = 1279 in
  let b = 6 in
  assert(extended_euclidean a b = (1, 1, (-213)));
;;
